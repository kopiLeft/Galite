 ...
 * Represents a form.
 */
abstract class ReportForm: Window() {
    ...
    /** the report object that will be generated */
    lateinit var reportObject : VReport
     ...
     *
     * @param ident                the menu label in default locale
     */
    fun menu(ident : String): Menu {
        val menu = Menu(ident)
        menus.add(menu)
        return menu
    }
    ...
    override val model: VReportSelectionForm by lazy {
        ...
        object : VReportSelectionForm() {
            ...
            override fun init() {
                source = sourceFile
                pages = this@ReportForm.pages.map {
                    it.ident
                }.toTypedArray()
