        ...
        if (field.hasNullableCols()) {
          for (j in 1 until field.getColumnCount()) {
            if (!field.getColumn(j)!!.nullable) {
              conditions!!.add(Op.build { field.getColumn(j)!!.column eq  field.getColumn(0)!!.column })
            }
          }
        } else {
          for (j in 1 until field.getColumnCount()) {
            conditions!!.add(Op.build { field.getColumn(j)!!.column eq  field.getColumn(j - 1)!!.column })
          }
        }
      }
