...
abstract class VFullCalendarBlock(form: VForm, val formBlock: FullCalendarBlock) : VBlock(form) {
   ...
   */
  fun queryEntries(week: Week): MutableList<VFullCalendarEntry> {
  ...
  fun doNotModalBlock(record: Int) {
    fullCalendarForm = formBlock.getFullCalendarFormModel(form)

    fullCalendarForm!!.transaction(Message.getMessage("loading_record")) {
