...
class FactoryCodePrinter: Constants {
   ...
   * @param lsdFactory If true, exclude the verification of blank strings.
  ...
  fun print(factory: Factory,
    ...
    printImports()
    printFactory(factory, lsdFactory)
  }
   ...
   */
  private fun addBodyFunction(classFactory: ClassFactory, lsdFactory: Boolean) {
    ...
    classFactory.attributes.forEach{ attribute ->
      ...
      if(attribute.Required) {
        ...
        if(!"String".equals(attribute.type) || lsdFactory) {
   ...
   */
  private fun printFactory(factory: Factory, lsdFactory: Boolean) {
      startFactory(factory.fullName)
      classesFactory.forEach {
        addCommentFunction(it)
        addBodyFunction(it, lsdFactory)
        if (it.hasChoiceBloc) {
