...
[source,kotlin]
----
class OrderedChart: Chart(title = "Ordered quantities per month", help = "TThis chart lists the ordered quantities per month", locale = Locale.UK) {
  ...
}
----
 ...
 * Add a command that calls the `createReport` method and pass the report as a parameter to this method.
