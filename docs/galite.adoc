...
Preface
-------
The Galite Project is the framework of kopiLeft for creating business applications. It's the successor of the Kopi project which is a framework from the company DMS that allows to development of database applications using Java, JDBC, SWING and Vaadin 7.
...
- UI: The UI is the presentation layer that represents the user interface. This can be extended to implement many technologies (web, mobile, desktop,.). For the moment Galite can generate a web application implemented in Vaadin flow (21+).
...
There is a list of standard database tables used by Galite inorder to manage application modules and users. you should create these tables in your first database migration.
...
- *MODULE*: This table contains the modules of our application. Example in our Menu bar we have `Client` Menu that contains the client form also we have `Bill` menu that contain two forms bill and bill product,
so in this case we need to insert 5 lines in our `Module` table.
