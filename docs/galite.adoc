...
==== MODULE
This table contains the modules of our application. Example in our Menu bar we have Client Menu that contains the client form also we have Bill menu that contain two forms bill and bill product,
...
|UC
|integer
|Number of changes applied on column in the database.

|TS
|integer
|Time stamp of last changes on column.
...
==== BENUTZERRECHTE
The table `User Rights` displays the modules that each user is allowed to use
...
==== GRUPPENRECHTE
This table displays the modules that each group of users is allowed to use
...
This table contains the groups that each user belongs to. Each user can belong to many groups
...
==== SYMBOLE
This table store the icons for each module
...
==== FAVORITEN
This table contains favorized modules for each user
...
==== KOPI_USERS
This table contains information about the users who use this application
...
==== DUMMY
This table is for database sequences
