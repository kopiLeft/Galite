...
AccessMode      : "arrayOf(" vararg ListOfModes.. ")" |  vararg ListOfModes.. |  mode = MODE
----
...
To create a block you need first to create a class that extend from Block class here we can specify the buffer size, the rows number and the name of the block as parameter, in second part you need to use insertBlock method that allow you to add the block to the form.

*Syntax*
[source,kotlin]
----
BlockDefinition :
class Block: Block(buffer: Int, rows: Int, blockName : String) {
    BlockBody
}
...
[source,kotlin]
----
class BlockCommand : Block(1, 5, "Commands") {
  ...
}
...
[source,kotlin]
----
class BlockCommand : Block(10, 5, "Commands") {
  ...
}
...
[source,kotlin] 
----
class Degree : Block(10, 5, "Degree Block")
----
...
[source,kotlin]
----
inner class Degree : Block(10, 5, "Degree Block") {
    init {
        border = Border.LINE
    }
}
...
[source,kotlin]
----
inner class School : Block(1, 1, "School") {
  val L = table(Lessons)
...
[source,kotlin]
----
inner class Degree : Block(1, 1, "Degree") {
...
[source,kotlin]
----
object Degree : Block(10, 5, "Degree block") {
