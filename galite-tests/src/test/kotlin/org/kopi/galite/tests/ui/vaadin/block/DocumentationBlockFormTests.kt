 ...
 /* @Test
  ...
  fun `test noDetail record of type LocalDateTime`() {
    ...
    for ( rec in 0 until form.noDetailBlock.buffer) {
      if (form.noDetailBlock.isCurrentRecordFilled() && form.noDetailBlock.trainDateLocalDateTime[rec] != null) {
        assertEquals(LocalDateTime::class.simpleName,
                     form.noDetailBlock.trainDateLocalDateTime[rec]!!::class.simpleName)
      }
    }
  }
