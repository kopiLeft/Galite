...
class ClientR : Report("Client Report", Locale.UK) {
  ...
  val fNameCl = field(STRING(20)) {
    label = "FirstName"
    help = "FirstName Client"
    align = FieldAlignment.LEFT
    format { v ->
      v.toUpperCase()
    }
  ...
  val lNameCl = field(STRING(20)) {
    label = "LastName"
    help = "LastName Client"
    align = FieldAlignment.LEFT
    format { v ->
      v.toUpperCase()
    }
  ...
  val telCl = field(INT(8)) {
    label = "Telephone"
    help = "Telephone Number"
    align = FieldAlignment.LEFT
  }

  val emailCl = field(STRING(20))
  {
    label = "Email"
    help = "Email"
    align = FieldAlignment.LEFT
  ...
  init {
    transaction {
      clients.forEach() { result ->
        add {
          this[idCl] = result[Client.idCl]
          this[fNameCl] = result[Client.firstNameCl]
          this[lNameCl] = result[Client.lastNameCl]
          this[telCl] = result[Client.telephoneCl]
          this[emailCl] = result[Client.emailCl]
        }
      }
    }
  }
