...
class OrdReceivedR : Report("Order Received Report", Locale.UK) {
  ...
  val idRec = field(INT(10)) {
    label = "ID"
    help = "ID Ord Received"
    align = FieldAlignment.LEFT
  }

  val total = field(DECIMAL(10, 5)) {
    label = "Total"
    help = "Total"
    align = FieldAlignment.LEFT
  }

  val ordRec = OrdReceived.selectAll()

  init {
    transaction {
      ordRec.forEach() { result ->
        add {
          this[idRec] = result[OrdReceived.idRec]
          this[total] = result[OrdReceived.total]
        }
      }
    }
  }
